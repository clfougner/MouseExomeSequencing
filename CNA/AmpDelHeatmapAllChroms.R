...
for(SN in sampleNames){
  filepath<-paste("/Users/christianfougner/Documents/Forskerlinja/DMBA-indusert/Sequencing/Output/AnalysisResults_w10k/Results/", SN, "/FastCallResults_", SN, ".txt", sep="")
  ...
  # Create data frame with 100kb bins along the entire chromosome
...
annotationDF<-data.frame(clusterDF, subtypesDF, mutFreqDF)
ha = HeatmapAnnotation(df = annotationDF, col = list(Subtype = colorsSubtype, Cluster=colorsCluster, MutationFrequency = colorRamp2(c(0, max(mutFreqDF[,1])), c("gray98", "turquoise4"))), gap = unit(0.5, "mm"))
...
pdf(file="/Users/christianfougner/Documents/Forskerlinja/DMBA-indusert/Sequencing/Output/test/CNAclusterplotw10k.pdf")
